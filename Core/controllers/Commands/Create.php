...
class Create extends ConsoleController
{
    ...
    public function createNonModuleController($controllerName = '', $addController = '', $location = 'Controllers')
    {
        ...
        if (str_contains($location, '--dir') && $location != 'Controllers') {
            $location = str_replace('--dir:', '', $location);
            $location = str_replace('_', ' ', $location);
            $location = ucwords($location);
            $location = str_replace(' ', '/', $location);
            $this->controllers = 'Controllers' . DS . $location;
    ...
    public function createNonModuleModel($modelName = '', $modelType = '--easy', $removeModel = '', $location = 'Models')
    {
        ...
        if ($modelDirectory && is_dir($modelDirectory)) {
            ...
            $modelType = ($modelType == 'models') ? $modelType : '--easy';
    ...
    public function createService($location = '', $serviceName = '', $defaultType = '--service')
    {
        ...
        if ($module[0] === 'empty') {
            $serviceDirectory = $this->createAppRootDirectory($this->service);
        }

        $serviceName = ucwords($serviceName);
        $serviceFile = $serviceDirectory . DS . $serviceName . $this->fileExtention;

        if (file_exists($serviceFile)) {
            $this->failureOutput(ucfirst($serviceName) . " Service exists already in the " . ucfirst($this->service) . " directory");
            return;
        }
    ...
    public function createAction($location = '', $actionName = '', $actionType = '--default', $removeAction = '')
    {
        ...
        $actionFile = $actionDirectory . DS . $actionName . $this->fileExtention;

        if (file_exists($actionFile)) {
            $this->failureOutput(ucfirst($actionName) . " Action exists already in the " . ucfirst($moduleName ?: $this->action) . " directory");
            return;
        }
    ...
    public function createLibrary($location = '', $libraryName = '', $defaultType = '--library')
    {
        ...
        if ($module[0] === 'empty') {
            $libraryDirectory = $this->createAppRootDirectory($this->library);
        }

        $libraryName = ucwords($libraryName);
        $libraryFile = $libraryDirectory . DS . $libraryName . $this->fileExtention;

        if (file_exists($libraryFile)) {
            $this->failureOutput(ucfirst($libraryName) . " Library exists already in the " . ucfirst($moduleName ?: $this->library) . " directory");
            return;
        }
    ...
    public function createHelper($location = '', $helperName = '', $helperType = '--base')
    {
        ...
        if ($module[0] === 'empty') {
            $helperDirectory = $this->createAppRootDirectory($this->helper);
        }
        ...
        $helperFile = $helperDirectory . DS . $helperName . $this->fileExtention;

        if (file_exists($helperFile)) {
            $this->failureOutput(ucfirst($helperName) . " Helper exists already in the " . ucfirst($moduleName ?: $this->helper) . " directory");
            return;
        }
     ...
     * @return void
     */
    public function createTap($name = '', $defaultType = '--default')
    {
        $tapName = $name;
        $created = '';
        ...
        $tapDirectory = $this->createAppRootDirectory($this->tap);

        $tapName = ucwords($tapName);

        if (file_exists($tapDirectory . DS . $tapName . $this->fileExtention)) {
            $this->failureOutput(ucfirst($tapName) . " Tap exists already in the Taps directory");
            return;
        }

        if ($tapDirectory && is_dir($tapDirectory)) {
            $filePath = $tapDirectory . DS . $tapName;
            $defaultType = str_replace('-', '', $defaultType);
            $created = $this->createFile($filePath, strtolower($defaultType) . '_tap', $this->tap);
        }

        if ($created) {
            $this->successOutput(ucfirst($tapName) . " Tap Class created successfully ");
            return;
        }
    }
